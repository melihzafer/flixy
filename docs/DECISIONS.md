...
**Decision:** Mobile reads exclusively from Supabase tables `titles` and `title_availability` (public-read RLS, writes blocked). TMDB ingestion lives in the server-only `@flixy/catalogue-ingest` package, suitable for Trigger.dev or a locked-down job runner with Supabase service-role credentials; `apps/mobile/src/lib/tmdb.ts` remains a non-imported guardrail stub. For dev, we seed 20 representative titles + ~15 availability rows in migration 0004 so the deck can be exercised pre-ingest.
...
## DEC-020: Upgrade Expo SDK 52 to 54 + Reanimated v4 worklets split
**Date:** 2026-01-15
**Context:** The latest Expo Go client ships SDK 54 only; SDK 52 projects can no longer connect without sideloading an older Go binary. Continuing on SDK 52 would force every contributor to manage parallel Expo Go installs.
**Decision:** Upgrade to Expo SDK 54 (Expo ~54.0.33, React Native 0.81.5, React 19.1.0, expo-router ~4.0.6). Reanimated v4 splits its Babel plugin into the new `react-native-worklets` package; `babel.config.js` now references `react-native-worklets/plugin` (kept last) and the package is added as an explicit dependency. The Sentry plugin string `@sentry/react-native/expo` is added to `app.config.ts`. A repo-level `.npmrc` adds `public-hoist-pattern` entries for expo / react-native / sentry / nativewind / react-native-css-interop / react-native-worklets so the Expo Metro pipeline can resolve auto-required packages under pnpm's isolated linker. SRS NFR-TECH-001 and PRD/FSD references bumped from 'SDK 52+' to 'SDK 54+'.
**Alternatives considered:** (a) Pin to SDK 52 and ask contributors to install the legacy Expo Go APK -- rejected, ongoing friction. (b) Switch to `node-linker=hoisted` -- rejected, EPERM errors on Windows during install.
**Reversibility:** Easy. Revert `apps/mobile/package.json` + `pnpm-lock.yaml` + the babel plugin one-liner + `.npmrc`.
