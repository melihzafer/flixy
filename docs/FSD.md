...
- **Mobile Client.** The user-facing application on iOS and Android. Responsible for rendering the swipe deck, handling gestures, caching, queueing offline writes, and capturing telemetry. Built on **React Native** (latest stable, New Architecture enabled) atop **Expo SDK 52+** with TypeScript strict mode; gestures and animations execute on the UI thread via `react-native-reanimated` v3 and `react-native-gesture-handler`. Stack and rationale are recorded in PRD § 16.5.5 and pinned in SRS § 7.5 (NFR-TECH-001..012).
