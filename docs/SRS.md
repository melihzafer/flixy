...
- **Mobile framework.** **React Native** (latest stable, New Architecture enabled) on **Expo SDK 54+**, with TypeScript in strict mode. This decision is locked; rationale tracked in PRD § 16.5.5 Technical Foundations and enforced via NFR-TECH requirements in § 7.5. Native swipe physics quality remains non-negotiable — the chosen framework must demonstrably deliver 60 fps gesture-driven animation on mid-tier hardware (Pixel 6a / iPhone 12 reference devices).
...
- **NFR-TECH-001 [P1, MUST].** The application SHALL be built with **React Native** (latest stable release at project initialization, with the New Architecture / Fabric enabled) on **Expo SDK 54 or later**. The project SHALL be configured via `expo-router` for file-based routing with the deep-link scheme `flixy://` per FSD § 5.3. Rationale: decision locked in PRD § 16.5.5; cross-platform iOS/Android delivery from a single TypeScript codebase.
