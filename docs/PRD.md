...
| 1.1 | 2026-04-26 | Melih | Renamed product SwipeReel → Flixy; added Appendix E (Brand Foundations) with typography pairing |
| 1.2 | 2026-04-26 | Claude Code | Locked mobile stack (React Native + Expo + full library set); added § 16.5.5 Technical Foundations; logged stack decision in Design Decisions Log |
...
### 16.5 Appendix E — Brand Foundations

This appendix records brand-level decisions that bind on Design and Engineering. Detailed token specifications live in the FSD (§ 4 UI/UX Specifications). Implementation requirements live in the SRS (§ 7.4 Brand Typography Requirements).

**Product name.** Flixy.

**Typography pairing (locked).**
- **Display / editorial:** GT Sectra (Grilli Type) — used for movie titles, screen heroes, and any surface where the type itself carries emotional weight. Italic cuts are preferred for title cards to evoke cinema-poster authority.
- **UI / body:** GT America (Grilli Type) — used for navigation, body copy, labels, controls, and any functional UI surface. Regular and Medium weights are the workhorses; Bold is reserved for emphasis.
- **Rationale:** The two families are designed by the same foundry to coexist. GT Sectra reads as cinematic and editorial without the SaaS-default associations of Playfair or PP Editorial. GT America has more character than Inter without sacrificing the readability needed for dense surfaces (watchlist grids, search results, settings).
- **Licensing:** Commercial app-embedded licenses must be procured from Grilli Type before public launch. Pre-launch / internal builds may use Fraunces (free, Google Fonts) as a temporary stand-in for GT Sectra; Inter as a stand-in for GT America. Stand-ins must be replaced before App Store submission.

**Aesthetic direction.** Dark-first (OLED-friendly true black), poster-as-hero composition, restrained chrome, springy tactile motion (200–350ms), subtle film-grain texture on dark surfaces. The reference vocabulary is "Criterion Collection meets premium product app" — neither indie-stencil nor generic-streaming-blue.

**Design Decisions Log.**

| Date | Decision | Alternatives considered | Rationale |
| --- | --- | --- | --- |
| 2026-04-26 | Product name: **Flixy** | SwipeReel (working name) | Shorter, more memorable, friendlier; locked in design tool. |
| 2026-04-26 | Typography: **GT Sectra + GT America** | PP Editorial New + Inter; Playfair Italic + Space Grotesk; Bebas Neue + Space Grotesk | Sharper editorial cuts pair better with poster-driven cards; designed-to-coexist family; differentiates from SaaS-default pairings. |
| 2026-04-26 | Mobile stack: **React Native + Expo (full stack locked)** | Native iOS (Swift) + Android (Kotlin); Flutter | TS/Supabase ecosystem fit; RN gesture physics proven at scale by Tinder/Bumble/Hinge; EAS Build/Update/Submit speeds release cadence and OTA hotfixes; single codebase fits a small team. |

### 16.5.5 Technical Foundations
...
**Design system tooling.** **NativeWind v4** (Tailwind for React Native) consumes design tokens defined in this document and the FSD. Color tokens are routed through a single `theme/colors.ts` module so the dark-first palette and the future light mode are one-file swaps. Typography is loaded via `expo-font` (GT Sectra + GT America in production builds; Fraunces + Inter as pre-license fallbacks per SRS NFR-BRAND-004).

**State and data.** **TanStack Query** owns all server state (caching, retries, optimistic updates, the offline mutation queue mandated by FSD § 3.6.4). **Zustand** owns ephemeral client/UI state (filter sheet, deck position, modal stack). **Supabase** (Postgres + Auth + Storage + Realtime for Phase 2) is the primary backend, with **BetterAuth** orchestrating session lifecycle on top.

**Observability and analytics.** **Sentry** for crashes and errors, **PostHog** for product analytics and feature flags (continuity with Melih's existing GitHands instrumentation), **Langfuse** for LLM call observability when Phase 2 "why this card" lands.

**Quality bar.** Biome (single-tool lint + format), Jest + Testing Library + MSW for unit/component tests, Maestro for end-to-end flows, husky + commitlint for commit hygiene. Coverage gate of 60% on business-logic modules (recommendation, swipe engine, watchlist) enforced in CI.

**Build and release.** EAS Build for iOS/Android binaries, EAS Update for OTA fixes of non-native code, EAS Submit for store delivery (gated on Apple Developer + Google Play account procurement; until then, internal builds via EAS preview profiles).

This stack is locked. Re-litigation requires a new entry in the Design Decisions Log with explicit reversal rationale.
