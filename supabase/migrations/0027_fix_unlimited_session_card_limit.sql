-- Fix card limit calculation for unlimited users in start_discovery_session:
-- If unlimited is true, card_limit must be null so that unlimited users have no card limit per session.
...
comment on function public.start_discovery_session(text, integer, text) is
  'Atomically enforces a rolling 1-hour session quota (fresh allotment once the window elapses) and creates one discovery card batch. Supports unlimited card_limit for unlimited plans.';
