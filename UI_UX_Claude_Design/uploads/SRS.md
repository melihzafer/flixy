...
- **Mobile framework choice.** A cross-platform framework (React Native + Expo or Flutter) is preferred to optimize a small team's velocity. Native swipe physics quality is non-negotiable; the chosen framework must demonstrably deliver 60 fps gesture-driven animation on mid-tier hardware.
...
1. Final mobile framework selection (React Native + Expo vs Flutter).
2. Streaming-availability provider selection (Watchmode vs JustWatch vs blended).
3. Recommendation engine v1 scope (rule-based + content-similarity vs early collaborative filter).
4. Real-time transport for Co-Swipe (Phase 2; Supabase Realtime vs dedicated service).
5. Payment processor for premium tier (StoreKit/Play Billing vs Stripe via web).
