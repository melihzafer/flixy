...
- **Mobile Client.** The user-facing application on iOS and Android. Responsible for rendering the swipe deck, handling gestures, caching, queueing offline writes, and capturing telemetry. Built on a single cross-platform codebase to maximize the small team's velocity.
...
This section enumerates the screens of the MVP and their behaviour. Visual styling is owned by Design; this document specifies functional behaviour and states.
...
- Title overlay at the bottom with: title, year, primary genre chips (max 2), runtime, age rating.
- Service badge row (the user's owned services that carry the title) — small logos, max 3 visible, with overflow "+2."
...
- Each row: thumbnail, title, year, runtime, primary service badge, overflow menu.
- Tap row: open detail.
...
- App links: `swipereel://title/{id}`, `swipereel://watchlist`, `swipereel://settings`, `swipereel://coswipe/{session}` (Phase 2).
- HTTPS universal links: `https://swipereel.app/title/{id}` etc.
...
- Mobile framework selection (React Native + Expo vs Flutter): tracked in engineering ADR.
